...
`harn-bitbucket-connector` is public so that it gets free hosted CI and so that
`harn add github.com/burin-labs/harn-bitbucket-connector@<tag>` resolves without
